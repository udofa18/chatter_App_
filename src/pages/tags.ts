...
const isInvalid = stringInput => {
	const inputs = Array.from(document.querySelector('.tags')?.children).map(input => (input.firstElementChild as HTMLElement)?.textContent);
...
document.querySelector<HTMLInputElement>("#tag")?.addEventListener("click", focus);
document.querySelector<HTMLTextAreaElement>("#tag")?.addEventListener("keyup", modifyTags);